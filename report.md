<h3 style="text-align:center">Interpreting Hidato as a Constraint Satisfaction Problem</h3>
**<p style="text-align:center"> Christian Lessard </p>**
**<p style="text-align:center">Robertson McClure </p>** <p style="text-align:center">Type of Project: CSP</p>
## Table of Contents
...
Our first approach for representing Constraints was as follows. Every cell on the board would have one constraint. That constraint would demand that if it has a value, then the precedent and subsequent numbers must be neighbouring or, alternatively, be next to empty spaces. This made our approach to constraint verification (to be discussed) difficult, so we decided upon a different type of constraint, opting instead to set each one as a relation between Variables. We started with an obvious structure, following the notion that "1 must be next to 2", "2 next to 3", and so on, but changed to a three way constraint along the lines of "2 needs 1 and 3 next to it", "3 needs 2 and 4 next to it", etc. This decision was made for reasons that will be discussed in Limitations and Obstacles.
...
define check(constraint):
  return next_to(constraint.start, constraint.middle) and
   next_to(constraint.middle, constraint.end)

define next_to(coord1, coord2):
  return abs(coord1[0] - coord2[0]) <= 1 and 
  abs(coord1[1] - coord2[1]) <= 1 and not coord1 == coord2
```

### Variable Selecting Heuristics

The function `bt_recurse` contains this section of code:
...
def bt_recurse(self, propagator, level):
      if not self.unasgn_vars:
          ...
          var = min([var for var in self.unasgn_vars 
          if not var.coord], key = self.heuristic)
...
To evaluate a solution, we wrote a polynomial time verifier function. This function, written as a method called “verify” in class CSP, begins with the variable encoding the starting number 1, and attempts to traverse the subsequent neighbours checking for their adjacencies on the board. If a valid path exists it returns "Success", else "Failure".

### Comparing Propagators

We tried various propagator functions. Our work features 33 puzzles in a file called `puzzle_bank.py`. They are organized by level of difficulty. The first 32 puzzles are all 6x6 grids, the last one is a 7x7 grid.

We plotted how long enough propagator took to solve a puzzle. We used a logarithmic scale because the difficulty of these problems increase exponentially (due to NP-Completeness). The logarithmic scale also is able to accentuate the differences in performance when the results are 0.001 of a second, and be able to compare it with results in the hundreds of seconds. The trend lines are exponential (which appear linear on a logarithmic scale).
...
It cannot be stated enough that this graph has a logarithmic scale. If we compare the two highest points (d6 and e1), it appears as if they took basically the same amount of time. However, d6 took around 200 seconds and e1 around 800 seconds. Please keep this in mind when viewing these graphs.
...
We had three heuristics at our disposal for testing.

- Smallest Current Domain
- Next In Line
- Random

Smallest Current Domain is the most intelligent heuristic, as it takes the variable that is most constrained, and therefore the variable that it is most likely to get the right placement.
...
Random was the only other heuristic we could think of that might inspire interesting results. However, we did not end up using random as it's performance on harder boards was so unsatisfactory (many hours) that we grew impatient and halted that entire experiment.

![Comparing Heuristics](./heuristics.png)

As you can see Smallest Current Domain greatly outperformed Next In Line, as the trend lines indicate. However, we believe that it is very interesting that on board d6, a very difficult board, Next In Line had an amazing performance. It outperformed Smallest Current Domain on the scale of several orders of magnitude: SCD 205 seconds, NIL 0.2 seconds. We believe this ridiculous performance is the result of that particular board setup. This heuristic happened to get lucky with every selection and the inherent drawbacks were cancelled out by the fact that it happened to place variables in the correct spots on the first try.

## Limitations and Obstacles

### Constraint Construction

Originally, we believed constraints could be defined in terms of two variables. This left out a few edge cases, leaving our CSP to generate incorrect solution, inspiring the changes mentioned earlier in the report.
...
We believe that difficulty could be represented as a ratio between board size and the number of given variables on the board, although this would take more thought. 
...
We learned that a well-planned CSP implementation will bare an enormous impact on the results of the problem. Hidato presents an interesting problem, and makes for a fun game. Later testing with different propators would be of the most interest to us. 

## Works Cited

1. Bellos, Alex. "Can You Solve It? Is Beehive Hidato the New Sudoku?" The Guardian. Guardian News and Media, 06 July 2015. Web. 05 Apr. 2017. Website
2. Figures 1 and 2, courtesy of Wikipedia, https://en.wikipedia.org/wiki/Hidato 